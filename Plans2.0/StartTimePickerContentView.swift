//
//  TimePickerContenrView.swift
//  DaApp
//
//  Created by Muhammed Demirak on 4/8/22.
//

import UIKit

class StartTimePickerContentView : UIView, UIContentView {
    
    struct Configuration: UIContentConfiguration {
        var start_time_val = Date()
        var onChange: (Date)->Void = { _ in } // onChange handler w/ default empty handler

        func makeContentView() -> UIView & UIContentView {
            return StartTimePickerContentView(self)
        }
    }
    ...
    func configure(configuration: UIContentConfiguration) {
        ...
        timePicker.date = configuration.start_time_val
    }
...
extension UICollectionViewListCell {
    func startTimePickerConfiguration() -> StartTimePickerContentView.Configuration {
        StartTimePickerContentView.Configuration()
    }
}
